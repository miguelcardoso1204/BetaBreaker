...
/**
 * Per-user monthly billing rate for gyms (EUR).
 *
 * Gyms pay €0.50 for each "active" user per month. A user is considered
 * active if they logged at least one ascent at that gym's routes during
 * the billing period. This rate is used by both the Postgres billing
 * function (server-side invoice generation) and the admin dashboard
 * (client-side projected bill display).
 */
export const GYM_BILLING_RATE_EUR = 0.50;

/**
 * Lifecycle statuses for gym billing records.
 *
 * Each billing record (one per gym per month) moves through:
 *   pending   — generated by the monthly cron job, awaiting invoicing
 *   invoiced  — invoice sent to the gym
 *   paid      — gym has paid the invoice
 */
export const GYM_BILLING_STATUSES = ['pending', 'invoiced', 'paid'] as const;
